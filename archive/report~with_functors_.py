...
class HttpressReportCollector:
    ...
    def report(self):
        ...
        # <editor-fold desc="loops with asserts">
        # new_lines = []
        #
        # num = len(functors)
        # assert num == len(self.lines)
        #
        ...
        #
        #     new_lines.append(
        #         [func[i].report(self.count, line[i])
        #                 for i in range(n)])
        # </editor-fold>
    ...
    def collect(self, report):
        ...
        if self.count == 0:
            ...
            # num = len(functors)
            # assert num == len(self.lines)
            # assert num == len(report.lines)
            #
            # for index in range(num):
            #     func = functors[index]
            #     s_line = self.lines[index]
            #     r_line = report.lines[index]
            #
            #     n = len(func)
            #     assert n == len(s_line)
            #     assert n == len(r_line)
            #
            #     new_lines.append(
            #         [func[i].collect(s_line[i], r_line[i])
            #                 for i in range(n)])
            # </editor-fold>
